...
function get(token, page , type=1 ,data) {
  var u = (new Date).getTime()
  var c = "12574478"
  if(type == 1) {
   n=`{"spm":"a21k45.26461518.order.3","tabCode":"waitSend","page":${page},"OrderType":"OrderList","templateConfigVersion":"0","appName":"tborder","appVersion":"3.0","ttid":"201200@taobao_h5_9.18.0","requestIdentity":"#t#ip#h5","condition":"{\\"version\\":\\"1.0.0\\"}","extParams":"{\\"useNewDetail\\":true}"}`
  }if (type==2) {
    data = JSON.parse(data)
    
    n=`{"platFormType":"h5","complaintSubjectType":"order","bizId":"${data.bizID}","sid":"${data.sid}","operationCode":"complaint_raise","complaintSid":"${data.sid}","params":"{\\"reasonCode\\":\\"11\\",\\"refererId\\":\\"\\",\\"raiseComplaintType\\":\\"\\",\\"complaintSubjectType\\":\\"order\\",\\"multiVersion\\":\\"\\",\\"subReasonCode\\":\\"${data.subtype}\\",\\"complaintTargets\\":[{\\"orderId\\":\\"${data.bizID}\\",\\"amount\\":${data.num},\\"itemId\\":\\"${data.itemId}\\"}],\\"proofTask\\":{\\"voucherList\\":[],\\"memo\\":\\"${data.memo}\\"},\\"contact\\":{\\"phoneNumber\\":\\"${data.phoneNumber}\\",\\"desensitized\\":true},\\"exts\\":{},\\"from\\":\\"\\"}"}`
    c='25663235'
  } 
  console.log(n)
  
  var sign = getsign(token + "&" + u + "&" + c + "&" + n)
  data = encodeURIComponent(n).replaceAll('_', "%5F").replaceAll('!', "%21")
  result =  {
    "sign": sign,
    "t": u,
    "data": data
...
// console.log( get('d9e1ad06e4be3b16e83125d7119ca683',1,2,JSON.stringify( {
//   bizID:'2204173561590486464',
//   sid:'80c4e180-0d41-413b-bba4-d8884c740f80',
//   itemId:'694679199683',
//   memo:'阿斯达阿斯达',
//   phoneNumber:'186****1380'
// })))
// console.log(get() )
// u = 1707230582130

// u = 1707280039970
function get_seach(token, publishSession, seach) {
  console.log(token);
  console.log(publishSession);
  console.log(seach);
  var data = `{"params":"{\\"ugcScene\\":\\"pc_newcreator_video\\",\\"publishVersion\\":\\"1\\",\\"site\\":\\"guangguang\\",\\"publishSession\\":\\"${publishSession}\\",\\"keyword\\":\\"${seach}\\",\\"cursor\\":\\"1\\"}"}`
  console.log(data);
  var sign = JSON.parse(get(token, data))
